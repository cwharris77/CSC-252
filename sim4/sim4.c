...
// CSC 252 Fall 2022
// This program simulates a single cycle proccessor
...
// This function is passed an instruction as input; it must read all of the fields out of the 
// instruction, and store them into the fields in the InstructionFields struct.
// It used bit shifting and masking to extract bits and set the fields correctly
void extract_instructionFields(WORD instruction, InstructionFields*fieldsOut) {
    // In all lines the hex value is the mask
...
// The first parameter is an in parameter: it is the InstructionFields struct that you filled in, 
// in the previous function. The second is a CPUControl* struct. Read the opcode and funct from the Fields 
// struct, and then set all of the correct controls in the Control struct. This function returns 1 if the 
// instruction is recognized, and 0 if the opcode or funct is invalid.
...
// a helper function that sets the fields of the CPUControl using the funct
// bits from the instruction. I'm not sure if this violates the rule that no other
// function can modify the CPU control fields because it is just a way to split up a 
...
// Reads the proper word out of instruction memory, given the current Program Counter. Returns the value.
