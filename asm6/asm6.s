...
.text
.globl sortInput
# This function is like main and will call the neccessary functions to get integers as keyboard input and sort them.
# It initially calls getLen to determine how long the array of ints will be then adds space on the stack for the array 
# and stores the keyboard input. It then calls a function to print the array, sort the array, and print the sorted array.
# After this it calls a function to decide whether the program is run again. 
#
# Registers:
#	s0: userLen, the length of user defined array
#	t0: i
#	t1-t4: various temporaries
#
sortInput:
...
.globl print_array
# This function takes a pointer to an int array and the length of the array as parameters.
# It then prints out the array.
#
# Registers:
#	s0: userLen
#	t0: i
#	t1-t2: various temporaries
#	t7: address of array
#
print_array:
	addiu $sp, $sp, -24			# standard prolouge 
...
.globl bubble_sort
# This function takes a pointer to an int array and the length of the array as parameters.
# It then uses bubble sort to sort the array in ascending order.
#
# Registers:
#	s0: userLen - 1
#	t0: i
#	t1: j
#	t2-t6: various temporaries
#
bubble_sort:
	addiu $sp, $sp, -24			# standard prolouge 
	sw $fp, 0($sp)
...
.globl run_again
# This function takes no parameters. It uses mips syscalls to ask for keyboard input
# in a dialog box with the question run again? If the answer is yes it runds the program again.
#
# Registers:
#	t0-t3: various temporaries
#
run_again:
	addiu $sp, $sp, -24			# standard prolouge 
	sw $fp, 0($sp)
